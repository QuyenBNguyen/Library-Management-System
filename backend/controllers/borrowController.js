...
exports.getAllBorrowBooks = async (req, res) => {
  try {
    ...
    let borrowBooks = await BorrowBook.find(query)
      .populate({ path: "book", select: "title author status" })
      .populate({
        path: "borrowSession",
      ...
      .skip((page - 1) * limit)
      .limit(Number(limit));
    // Filter by book.status if status is provided
    if (status) {
      borrowBooks = borrowBooks.filter(bb => bb.book && bb.book.status === status);
    }
