...
exports.checkoutBooks = async (req, res) => {
  try {
    ...
    const borrowBooks = await Promise.all(bookIds.map(async (bookId) => {
      ...
      const borrowBook = await BorrowBook.create({ book: bookId, borrowSession: session._id, isPaid: false });
      return borrowBook;
    }));
...
exports.getAllBorrowBooks = async (req, res) => {
  try {
    ...
    let borrowBooks = await BorrowBook.find(query)
      ...
      .populate({ path: 'borrowSession', populate: { path: 'member', select: 'name email' } })
      .skip((page - 1) * limit)
      .limit(Number(limit));
    // Filter by search if needed
    if (search) {
      borrowBooks = borrowBooks.filter(b =>
        (b.book?.title?.toLowerCase().includes(search.toLowerCase()) ||
         b.borrowSession?.member?.name?.toLowerCase().includes(search.toLowerCase()) ||
         b.borrowSession?.member?.email?.toLowerCase().includes(search.toLowerCase()))
      );
    }
    res.status(200).json({
      data: borrowBooks,
...
exports.getMyBorrowBooks = async (req, res) => {
  try {
    ...
    const borrowBooks = await BorrowBook.find({ borrowSession: { $in: sessionIds } })
      .populate({ path: 'book' })
...
exports.getMyBorrowingBooks = async (req, res) => {
  try {
    ...
    const borrowBooks = await BorrowBook.find({ borrowSession: { $in: sessionIds } })
      .populate({ path: 'book' })
      .populate({ path: 'borrowSession', select: 'dueDate borrowDate' });
    const result = borrowBooks.filter(bb => !bb.returnDate && bb.book && bb.book.status === 'checked out');
...
exports.getMyReservingBooks = async (req, res) => {
  try {
    ...
    const borrowBooks = await BorrowBook.find({ borrowSession: { $in: sessionIds } })
      .populate({ path: 'book' })
      .populate({ path: 'borrowSession', select: 'dueDate borrowDate' });
    const result = borrowBooks.filter(bb => !bb.returnDate && bb.book && bb.book.status === 'reserved');
    res.status(200).json(result);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
exports.getMyOverdueBooks = async (req, res) => {
  try {
    ...
    const borrowBooks = await BorrowBook.find({ borrowSession: { $in: sessionIds } })
      .populate({ path: 'book' })
      .populate({ path: 'borrowSession', select: 'dueDate borrowDate' });
    const result = borrowBooks.filter(bb => {
      if (bb.returnDate) return false;
      const due = bb.borrowSession?.dueDate;
      return bb.book && bb.book.status === 'checked out' && due && new Date(due) < now;
    }).map(bb => {
      return { ...bb.toObject() };
    });
    res.status(200).json(result);
...
exports.createBorrowBook = async (req, res) => {
  try {
    const { book, borrowSession, returnDate, overdueDay, fineAmount, isPaid } = req.body;
    const borrowBook = await BorrowBook.create({ book, borrowSession, returnDate, overdueDay, fineAmount, isPaid });
...
exports.updateBorrowBookById = async (req, res) => {
  try {
    const borrowBook = await BorrowBook.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (borrowBook) {
      res.status(200).json(borrowBook);
    } else {
      res.status(404).json({ message: 'BorrowBook not found' });
...
async function updateOverdueFinesForUser(userId) {
  ...
  const sessionIds = sessions.map(s => s._id);
  const now = new Date();
  const borrowBooks = await BorrowBook.find({ borrowSession: { $in: sessionIds } }).populate('borrowSession');
  for (const bb of borrowBooks) {
    if (!bb.returnDate && bb.borrowSession && bb.borrowSession.dueDate && new Date(bb.borrowSession.dueDate) < now) {
      const overdueDay = Math.max(0, Math.ceil((now - new Date(bb.borrowSession.dueDate)) / (1000 * 60 * 60 * 24)));
...
exports.cleanupExpiredReservations = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: `Cleaned up ${count} expired reservations.` });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
}; 